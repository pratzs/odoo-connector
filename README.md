...
**Tracking active overrides (dashboard "Active overrides" list under the Manual Price Override box, `GET /clearance/manual_prices`).** The SKU/price boxes clear after every Apply, so setting overrides on several SKUs in a row makes them easy to lose track of. `list_manual_price_overrides(shop_url)` lists every `ClearanceMirror` row with a non-null `manual_price` (base SKU, price, whether the mirror is currently active or drafted) — shown on page load and refreshed after every Apply/Clear, each row with an **Edit** (re-fills the SKU/price boxes) and **Clear** (removes the override immediately, no need to re-type the SKU) button.
...
| 69 | **Track active manual price overrides so they aren't forgotten** | Merchant noted that setting a manual override on one SKU clears the SKU/price boxes for the next one — with several SKUs to set, there was no way to see what had already been pinned or come back to change it. New `list_manual_price_overrides(shop_url)` (services/clearance.py) lists every `ClearanceMirror` row with a non-null `manual_price`, wired to `GET /clearance/manual_prices` and an "Active overrides" list under the Manual Price Override control — shown on page load and refreshed after every Apply/Clear, each row with **Edit** (re-fills the SKU/price boxes for that row) and **Clear** buttons so nothing set gets lost track of |
