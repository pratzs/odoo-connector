...
| 63 | **Fix: delivery-address child contact created with a blank street** | `find_or_create_child_address` (used by order sync to create/reuse the delivery-address child under a customer) read `address_data.get('street')`, but Shopify's address objects use `address1`/`address2`, not `street`/`street2` — the street line was always blank while city/zip/country_code came through fine (those happen to share Shopify's field names). Also meant the dedup search (`street = None` for every customer) could match and reuse the *first* blank-street child ever created for a repeat customer regardless of the new order's actual address. Fixed to read `address1`/`address2` (with a `street`/`street2` fallback for any other caller). Only affects new orders going forward — existing blank-street contacts in Odoo aren't retroactively corrected; a repeat customer's next order will create a new, correct child address alongside the old blank one rather than overwrite it |
