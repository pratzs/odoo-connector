...
def trigger_fix_variants():
    ...
    # Imports inside the function to avoid circular errors
    from services.products import fix_variant_mess_task 
    from models import Shop
    from utils import q_default
    
    # 1. Fetch Company ID safely from DB
    shop = Shop.query.filter_by(shop_url=shop_url).first()
    if not shop or not shop.odoo_company_id:
        return jsonify({"error": "No Odoo Company configured. Please save Settings."}), 400
    
    # 2. Enqueue the task, passing the Company ID explicitly
    q_default.enqueue(fix_variant_mess_task, shop_url, shop.odoo_company_id, job_timeout=3600)
